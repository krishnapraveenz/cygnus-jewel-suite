...
import { Printer, Building2, Hash, ShieldCheck, RotateCcw, Save, SlidersHorizontal, Gem, Blocks, Lock, Download } from "lucide-react";
...
import { checkForUpdate, downloadAndInstall, type UpdateStatus, type DownloadProgress } from "@/lib/updater";
...
function UpdateSection() {
  ...
  return (
    <div className="space-y-2 border-t border-border pt-5">
      <div className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Software updates</div>
      <div className="flex items-center gap-3">
        <Button size="sm" onClick={doCheck} disabled={checking || installing}>
          <Download className="w-3.5 h-3.5 mr-1" /> {checking ? "Checking…" : "Check for updates"}
        </Button>
        {status && !status.available && (
          <span className="text-xs text-success">You're on the latest version.</span>
        )}
      </div>
      {status?.available && !installing && (
        <div className="rounded-md border border-primary/30 bg-primary/5 px-3 py-2 space-y-1">
          <div className="text-sm font-medium">Version {status.version} available</div>
          {status.notes && <div className="text-xs text-muted-foreground">{status.notes}</div>}
          <Button size="sm" onClick={doInstall}>
            <Download className="w-3.5 h-3.5 mr-1" /> Download & install
          </Button>
        </div>
      )}
      {installing && progress && (
